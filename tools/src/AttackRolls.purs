...
viewAttackRoll attack =
  ...
  [ HE.div [ HA.class' "small grid" ]
    [ HE.div [ HA.class' "small s5" ] [ mkText "Description" (\s -> Attack (AUpdate attack (attack { description = s }))) attack.description ]
    , HE.div [ HA.class' "small s1" ] [ mkCheckbox "Advantage" "arrow_upward" "bottom" (\b -> Attack (AUpdate attack (attack { advantage = b}))) attack.advantage ]
    , HE.div [ HA.class' "small s1" ] [ mkCheckbox "Disadvantage" "arrow_downward" "bottom" (\b -> Attack (AUpdate attack (attack { disadvantage = b}))) attack.disadvantage ]
    , HE.div [ HA.class' "small s2" ] [ mkNumber "Attack Bonus" (\b -> Attack (AUpdate attack (attack { attackBonus = b }))) (Just attack.attackBonus) ]
    , HE.div [ HA.class' "small s2" ] [ mkNumber "Target AC" (\b -> Attack (AUpdate attack (attack { targetAC = b }))) (Just attack.targetAC) ]
    ...
    , HE.div [ HA.class' "small s11 fill grid middle-align" ] 
      ...
      , HE.div [ HA.class' "small s1" ] [ mkButton (Attack (AUpdate attack (attack { damageDice = snoc attack.damageDice initDamageDice }))) "add" "Add Attack Roll" ]
      ]
...
viewDice attack damage =
  ...
  [ HE.div [ HA.class' "small s2" ] [ mkNumber "Dice Count"   (\d -> Attack (AUpdate attack (attack { damageDice = updateArray attack.damageDice damage (damage { diceCount   = d }) } ))) (Just damage.diceCount  ) ]
  , HE.div [ HA.class' "small s2" ] [ mkNumber "Dice Type"    (\d -> Attack (AUpdate attack (attack { damageDice = updateArray attack.damageDice damage (damage { diceType    = d }) } ))) (Just damage.diceType   ) ]
  , HE.div [ HA.class' "small s2" ] [ mkNumber "Damage Bonus" (\d -> Attack (AUpdate attack (attack { damageDice = updateArray attack.damageDice damage (damage { damageBonus = d }) } ))) (Just damage.damageBonus) ]
  , HE.div [ HA.class' "small s2" ] [ mkText   "Damage Type"  (\d -> Attack (AUpdate attack (attack { damageDice = updateArray attack.damageDice damage (damage { damageType  = d }) } ))) damage.damageType         ]
  , HE.div [ HA.class' "small s3" ] [ HE.text ("Rolls: " <> show damage.damageRoll) ]
  , HE.div [ HA.class' "small s1" ] [ mkButton (Attack (AUpdate attack (attack { damageDice = delete damage attack.damageDice }))) "delete" "Remove Saving Throw" ]
  ]
